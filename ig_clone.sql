...
WHO WON??!!*/
SELECT u.username, p.id, p.image_url, l.Total_Likes
FROM (
...
SELECT u.username, GROUP_CONCAT(c.comment_text) AS comment_texts
FROM users AS u
LEFT JOIN comments AS c ON u.id = c.user_id
GROUP BY u.id, u.username
HAVING comment_texts IS NULL OR comment_texts = '';
...
SELECT
    ...
    (tableA.total_A / total_users.total_count) * 100 AS 'Percentage of Users who never commented',
    tableB.total_B AS 'Number of Users who commented on photos',
    (tableB.total_B / total_users.total_count) * 100 AS 'Percentage of Users who commented on photos'
FROM (
    ...
    FROM (
        ...
        LEFT JOIN comments ON users.id = comments.user_id
        GROUP BY users.id
        HAVING COUNT(comments.comment_text IS NULL) > 0
    ) AS total_number_of_users_without_comments
) AS tableA
JOIN (
    ...
    FROM (
        ...
        LEFT JOIN comments ON users.id = comments.user_id
        GROUP BY users.id
        HAVING COUNT(comments.comment_text IS NOT NULL) > 0
    ) AS total_number_users_with_comments
) AS tableB
CROSS JOIN (
